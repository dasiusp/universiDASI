...
A partir daqui, vamos começar à montar nosso CRUD. Vamos usar como exemplo uma lista para nossa aplicação, que ainda vai funcionar sem problemas! Mas normalmente é feita uma conexão com um banco de dados e executado os devidos comandos!
...
app.get("/pegar-informacao", (req, res) => {
    ...
    // o nome da rota ("/pegar-informacao") não pode ter espaços e acentos, exceto o hífens, sempre 
...
- **Body**: o conteúdo que pode ser enviado como resposta, podendo ser um json ou um html, por exemplo:
    ...
    <h1> Texto <h2>
    ```
Uma coisa importante: nem todas as repostas tem corpo, como as de status 204 e 304 (esta última ainda pode ter o body, mas não é obrigatório).
...
const app = express();
app.use(express.json()); // Aqui vai permitir enviar um objeto JSON pelo corpo e como resposta ({objeto: "isto é um objeto"})
...
Mas como sabemos que está dando certo? Primeira, temos que botar esse servidor para rodar! Vá no terminal e na pasta que está nosso index.js, rodamos `node index.js`, a partir daí ele vai ouvir a porta 8000.
